...
;; Custom TeX class for pdf exports from org mode
...
;; set up spelling checks within org mode with flyspell
(require 'ispell)
...
(defun connect-home ()
  (interactive)
  (dired "/ssh:mtoninel@node-map.cluster.ifom.eu:/storage/home/mtoninel"))

;; Configure connection to univa-comp cluster
(defun connect-storage ()
  (interactive)
  (dired "/ssh:mtoninel@node-map.cluster.ifom.eu:/storage/"))

;; function to open an eshell buffer directly loaded into node-map trough tramp by setting def dir to ssh
(defun map-shell ()
    (interactive)
   (let ((default-directory "/ssh:mtoninel@node-map.cluster.ifom.eu:/storage/")(tramp-encoding-shell "/bin/sh")) (eshell))
      (eshell))

;; Set configuration for projectile
(use-package projectile
 ...
 ;; (when (file-directory-p "/ssh:mtoninel@node-map.cluster.ifom.eu:/storage/home/mtoninel/map/")
 ;;   (setq projectile-project-search-path '("/ssh:mtoninel@node-map.cluster.ifom.eu:/storage/home/mtoninel/map/")))
